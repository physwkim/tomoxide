...
`crates/tomoxide-gui` built on [siplot] (egui 0.34 + wgpu scientific plotting,
a silx port) for every scientific view, and on siplot's sibling crate `sidm`
...
[siplot]: https://github.com/physwkim/siplot
...
composition of siplot widgets around tomoxide's actual API surface.

---

## 1. Application shell

An `eframe` application (wgpu renderer — required by siplot) with:
...
No docking framework in v1 — plain egui panels; siplot's detached windows
...
- **Metadata card**: nproj / nz / nx / nflat / ndark, theta start/end/step,
  raw size estimate.
- **Projection browser**: siplot `StackView` over the projections. Its
...
- **Sinogram inspector**: pick a detector row → `Plot2D` of the
  `[nproj × nx]` sinogram.
...
- **Right (dominant)**: siplot `CompareImages` — the current preview as
  image B against a **pinned** previous preview as image A. A "Pin" button
  (or holding Space) snapshots the current result together with its
  parameters; split/subtract/A-B modes give instant parameter comparison.
  The toolbar hosts `ColormapDialog` (histogram + vmin/vmax/normalization),
  rectangle-ROI stats, and line profiles.
...
- **3D**: siplot `ScalarFieldView` (isosurface) over a worker-built
...
- **Connection panel**: PVA channel addresses for projection / dark / flat /
  ...
  `sidm` data engine (its own tokio runtime). Frames land in a
...
- **Views**: `Plot2D` panes with crosshair cursors marking the other slice
  ...
  NaN-line hack: siplot draws real markers).
...
siplot is `edition = "2024"`, `rust-version = "1.92"`; the tomoxide workspace
...
If `tomoxide-gui` were a workspace **member**, siplot would enter the
...
- `siplot` / `sidm`: **git dependencies pinned to a tag/rev**, with a
  commented-out `[patch]` block pointing at the sibling checkout for local
  development — CI stays reproducible without requiring the sibling repo.
- `eframe = "0.34"` with the wgpu renderer (siplot deliberately does not
  depend on eframe itself; the app owns the window).
- **wgpu duplication**: tomoxide's optional `gpu-wgpu` backend pins wgpu 30
  while siplot renders through egui-wgpu 0.34's re-export (a different
  major). The two coexist (types never cross — the recon `Engine` owns its
  own instance; siplot renders through eframe's `RenderState`), but at a
...
crates/tomoxide-gui/
  ...
  src/
    ...
    live/
      ...
      source.rs         # sidm engine + PVA channels -> ring buffer
...
```text
UI thread (eframe/egui, all siplot widgets)
   ...
   └── live: sidm engine (tokio) ─ PVA frames ─► ring buffer ─► worker loop
```
...
- **M1 — MVP (offline preview)**: crate scaffold with the
  ...
  single-Z preview, ColormapDialog, ROI/profile, A/B CompareImages;
...
- **M2 — Full offline**: `--progress-json` (#4); Run screen with subprocess
  ...
  export); batch queue.
- **M3 — Live streaming**: sidm PVA source + ring buffer + Z-slice live

...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ✅ **Fft** (batched radix-2 1-D + 2-D) WGSL port (`fft.wgsl` bit-reversal +
    per-stage butterflies; `fft_transpose.wgsl` for the 2-D column pass). Power-
    of-two only (radix-2); non-power-of-two errors out → CPU fallback. Tolerance
    parity vs rustfft (GPU cos/sin twiddles); fft_1d & fft_2d roundtrip tests.
  ...
  - ⬜ Remaining: sub-pixel rotation-center phase in the filter
    (`fbp_filter_center`; CPU currently centers in the back-projector); then
    Bluestein for non-power-of-two FFT (would unblock GPU `fourierrec`/`lprec`).
    (`RayProject` is host-side sparse rows for ART/BART — not a GPU kernel.)

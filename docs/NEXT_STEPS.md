...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ✅ Rotation-center phase (`fbp_filter_center`) — resolved as an **intentional
    mechanism deviation, verified equivalent** (signed off). tomocupy folds the
    center into the shared FBP filter as a per-row Fourier phase and back-projects
    at `n/2`; tomoxide keeps `apply` a pure ramp and handles the center downstream
    where each method already touches the data (fbp/linerec at `t=…+center`;
    fourierrec/lprec/gridrec via their own signed-freq Fourier recenter). Not a
    defect — the downstream path matches the tomopy goldens (gridrec r=0.98,
    `gridrec_subpixel_center` non-integer regression). Documented in the
    `FbpFilter::apply` trait contract + PORTING.md; the only numerical gap left is
    edge-vs-zero padding in `apply` (small ramp-ringing difference, deferred).

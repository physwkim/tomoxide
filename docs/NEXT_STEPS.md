...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ⬜ Remaining: **`lamino`** wgpu port (USFFT-based, reference
    `~/codes/tomocupy/src/cuda/cfunc_{usfft1d,usfft2d,fft2d}.cu` — no CPU parity
    ref, so cross-method/phantom verification like fourierrec's, not CUDA golden);
    ...
    parity-verified on wgpu; fourierrec + lprec are the USFFT/log-polar recons
    ported.**

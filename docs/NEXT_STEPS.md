...
- **M4 — CUDA backend** (parity: tomocupy): 🟢 C-ABI shim over `cfunc_*` + CUDA
  runtime helpers, `nvcc` gated on the `cuda` feature. GPU FBP back-projection
  (`cfunc_linerec`) is live as `CudaBackend`'s `FilteredBackproject` (filter on
  the CPU via the shared definition); CUDA↔CPU Pearson = 1.0 on an RTX 5000 Ada
  (`cuda_fbp_parity.rs`). Remaining: GPU `fourierrec`/`lprec`/`cfunc_filter` and
  GPU elementwise/stripe/phase.

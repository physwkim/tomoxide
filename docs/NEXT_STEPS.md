...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ⬜ Remaining: **`lprec`** (log-polar — still silently routed through the FBP
    path, a known divergence) and **`lamino`** wgpu ports (USFFT-based, reference
    `~/codes/tomocupy/src/cuda/cfunc_{lprec,usfft*}.cu` — no CPU parity ref, so
    cross-method/phantom verification like fourierrec's, not CUDA golden);
    ...
    parity-verified on wgpu; fourierrec is the first full USFFT recon ported.**

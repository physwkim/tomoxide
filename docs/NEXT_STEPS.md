...
to-do list; the per-module upstream map lives in [`PORTING.md`](PORTING.md).
Every task below cites the
...
This is the milestone that makes tomoxide usable on **real** data. Goal:
...
- **Zarr writer — done** (`create_writer` for `Zarr`; `tests/` in
  `tomoxide-io`). Pure-Rust, **no new dependency**: a spec-compliant Zarr v2
  `DirectoryStore` (`{path}.zarr/exchange/data`, hand-written
  `.zgroup`/`.zarray`/`.zattrs`, one raw LE-f32 chunk per z-slice), readable by
  the Python `zarr` library. Deviates from tomocupy's Blosc-compressed
  multiscale NGFF pyramid (would need a zarr crate + Blosc C-binding, against
  the pure-Rust I/O stack); uncompressed single-scale stores identical values,
  Blosc + multiscale deferred.
...
- **M4 — CUDA backend** (parity: tomocupy): ✅ C-ABI shim over `cfunc_*` + CUDA
  runtime helpers, `nvcc` gated on the `cuda` feature. Implemented & verified on
  an RTX 5000 Ada: GPU FBP filter (`cfunc_filter`) + back-projection
  (`cfunc_linerec`) and `fourierrec` (`cfunc_fourierrec`) as a **device-resident
  fused** path (`AnalyticReconstruct`, one upload/one download, Δ=0 vs per-stage);
  a cuFFT **`Fft`** capability that composes `gridrec`/`lprec`/Paganin **phase**
  onto CUDA (rel ≈ 1e-6 vs CPU); GPU **elementwise** (dark/flat + minus-log).
  **Only remaining GPU gap — stripe removal:** by design `remove_stripe` is
  CPU-only (takes no backend; its internal FFT/wavelet/sort/CG kernels are not a
  backend capability), and the Vo/Ti methods (sorting, median, CG solve) are
  poorly GPU-suited. GPU-ifying it would mean refactoring `remove_stripe` to be
  backend-aware and porting per method (only the Fourier-wavelet `Fw` path would
  compose from `Fft`); deferred as low value vs the refactor risk.
- **M5 — Streaming pipeline:** 🟢 `crates/tomoxide/src/pipeline.rs`
  `ReconSteps::run` ports tomocupy `recon_steps_all` — read → normalize → phase,
  then reconstruct + `write_chunk` by sinogram (z) chunks (Δ=0 vs full
  `reconstruct`, `recon_steps_parity.rs`). Remaining: overlapped double-buffered
  read/compute/write threads and true out-of-core chunked *reads*.
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ✅ **Bluestein** chirp-z transform (`bluestein.wgsl`): `fft_1d` now handles
    **any length**, not just power-of-two — non-pow2 lengths run a length-n DFT
    ...
    3/5/6/12/100.
  - ✅ **Arbitrary-dim `fft_2d`** (separable Bluestein): power-of-two `rows`/`cols`
    keep the on-device fast path (row pass + transpose + row pass + transpose,
    all radix-2); any other dims take a separable fallback — a 1-D pass along the
    contiguous `cols` axis, a host transpose so `rows` become contiguous, a 1-D
    pass along `rows` (each pass radix-2 or Bluestein per length), and a transpose
    back. The per-axis inverse divisors (1/cols then 1/rows) compose to the 2-D
    1/(rows·cols), so it matches the CPU backend exactly (no new kernel — pure
    orchestration over `fft_1d` + a per-image host transpose). Test covers
    pow2×non-pow2 (4×6), non-pow2×pow2 (6×4), non-pow2² (3×5, 6×10): forward rel
    ≈1e-7, roundtrip ≈3–7e-7 vs rustfft. **Both `fft_1d` and `fft_2d` now handle
    any length like the CPU backend**, unblocking GPU `fourierrec`/`lprec`, which
    need arbitrary-length transforms.
  ...
  - ✅ **fourierrec** (tomocupy Gaussian-USFFT Fourier reconstruction): faithful
    port of `cfunc_fourierrec.cu`/`kernels_fourierrec.cuh` — centred 1-D FFT →
    separable-Gaussian gather onto a `(2n+2m)²` grid (`m=4`, `mu` from
    `eps=1e-3`) → border `wrap` → centred 2-D inverse FFT → Gaussian
    deapodization + crop + disk mask. Takes pre-filtered data (the dispatcher
    runs `FbpFilter::apply` first, then branches before the back-projector), so
    it was promoted out of the silent FBP alias. Needs only `Fft`, so it
    composes onto wgpu for free; n=96 drives the Bluestein 1-D + separable-
    Bluestein 2-D GPU paths end to end. No CUDA golden offline — verified by
    fourierrec↔phantom r=0.9713 (== gridrec) and fourierrec↔gridrec r=1.0000,
    GPU↔CPU NRMSE 3.2–3.5e-7. Two tomocupy geometry conventions adapted to
    tomoxide's (`+sin` gather y, symmetric crop), each verified vs the phantom.
    (`linerec` is faithfully covered by the FBP path — its CUDA kernel reduces to
    parallel-beam back-projection.)
  - ✅ **lprec** (tomocupy log-polar / Andersson–Carlsson–Nikitin recon): faithful
    port of `lprec.py` (precompute) + `cfunc_lprec.cu`/`kernels_lprec.cuh`
    (runtime) — replaces the prior silent FBP aliasing. `Nspan=3` overlapping
    `2β` spans, log-polar grids, the zeta convolution kernel `fZ` (discretization
    weights + magic const, 2-D fftshift to standard FFT order, B3 deapodize,
    Hermitian-extended to emulate cuFFT R2C/C2R with a C2C `fft_2d`), `C2lp`/
    `lp2p`/`lp2p_w` index sets; runtime = cubic-B-spline prefilter (pole `√3−2`)
    → 4-tap cubic gather polar→log-polar → 2-D FFT × `fZ` × inverse → cubic gather
    log-polar→Cartesian. Takes pre-filtered data (`fbp_filter_center` →
    `LpRec.backprojection`), so it branches after `FbpFilter::apply`; needs only
    `Fft` (pow2 at n=128), composes onto wgpu for free. One geometry convention
    adapted: output row coord is the value-negation of tomocupy's `x2*(-1)`
    (`lin[i]+1/N`, the true y-flip co-registering with tomoxide's `+sin`
    projector; index-mirroring mis-registers by ≈1 px) — verified vs phantom. No
    CUDA golden offline — verified by lprec↔phantom r=0.9348, lprec↔gridrec
    r=0.9676 (genuinely different inversions, ~0.97 not ~1.0), GPU↔CPU NRMSE
    3.0e-7. **lprec was the last silent recon-alias — every tomocupy recon
    algorithm is now either a real port or faithfully covered by the FBP path.**
  - ✅ **`lamino`** (Fourier USFFT laminography, `LamFourierRec`, arxiv
    2401.11101) — CPU + wgpu done. Intrinsically 3-D, so its own entry point
    `recon::lamino::lamino` (`[nproj,nz,n]`→`[rh,n,n]`), not the per-slice
    `recon` dispatch. Three chained Gaussian-gridding USFFT operators: `fft2d_fwd`
    (centred 2-D FFT of each ramp-filtered projection) → `usfft2d_adj` (scatter
    `(θ,kx,ky)` into the in-plane `(x,y)` frequency volume via `take_x`:
    `x=ku·cosθ+kv·sinθ·cosφ`, `y=ku·sinθ−kv·cosθ·cosφ`) → `usfft1d_adj` (transform
    along tilt-z to the real volume), `φ=π/2+lamino_angle·π/180`. Matched forward
    projector `lamino_project` (exact transpose chain) for self-consistent tests.
    Two pure-optimization deviations vs CUDA: drop chunking/double-buffering, and
    drop the R2C Hermitian-half (carry full complex spectra) — gridding geometry
    and per-op FFT normalization (cuFFT unnormalized inverse) identical, so
    numerically equivalent for real input while every operator is a clean
    transpose. Needs only `Fft`, composes onto wgpu for free (n,rh,nz pow2 →
    radix-2). No CUDA golden offline — verified by `fft2d_inv(fft2d_fwd)==id`
    (max\|Δ\|<1e-4), usfft1d/usfft2d fwd/adj adjoint dot-product (rel<1e-4), 3-D
    two-sphere phantom forward-project→reconstruct Pearson 0.9520 (confirms the
    take_x geometry end to end, not just the transpose), GPU↔CPU NRMSE 1.5e-7.
  - ✅ Rotation-center phase (`fbp_filter_center`) — **faithful refactor done**
    (signed off "Full faithful tomocupy"). `FbpFilter::apply` now folds the center
    into the shared filter as a per-row signed-frequency Fourier phase
    `exp(-2πi·f_k·(n/2−center)/pad)` (CPU + wgpu identical), so the analytic
    back-projectors and Fourier grids all reconstruct against a `center = n/2`
    geometry: fbp/linerec back-project at `n/2` (a `center=ncols/2` geometry),
    fourierrec/lprec are center-agnostic (their own in-grid recenter removed).
    gridrec keeps its own integrated filter+recenter; the iterative back-projectors
    keep `geom.center` (matched adjoints, unfiltered). δ=0 at the default center
    leaves the centered goldens unchanged; off-center is verified by
    `center_in_filter_parity.rs` (fbp/fourierrec/lprec recover an axis-at-66.5
    sub-pixel phantom and match the centered recon) + the e2e pipeline (center
    63.5 → FBP r=0.87). Signed frequency is mandatory — a raw index collapses a
    half-integer center. Documented in the `FbpFilter::apply` trait contract +
    PORTING.md.
  - ✅ Edge-replicate padding (`fbp_filter_center` second half) — **done**.
    `apply` (CPU + wgpu) now centres each lane in an `ne = (4·n).next_power_of_two()`
    buffer and edge-replicates both borders (`tmp[:pad]=data[:1]`,
    `tmp[pad+n:]=data[-1:]`), then crops the centred `[pad:pad+n]` window —
    matching tomocupy's width, placement, and fill, so the long-tailed ramp no
    longer rings against a hard zero step. `make_fbp_filter` length went
    `(2·n)`→`(4·n).next_power_of_two()` (== tomocupy float32 `ne=4·n` for every
    power-of-two width). No remaining padding gap in `apply`.
  - ⬜ Remaining: (`RayProject` is host-side sparse rows for ART/BART — not a GPU
    kernel.) **All GPU-appropriate capability traits are implemented and
    parity-verified on wgpu; fourierrec + lprec + lamino are the USFFT/log-polar
    recons ported.**
- **M7 — Beam hardening, AI center (`find_center_sift`), f16, benchmarks.**
  (Zarr writer landed — see the I/O section above. Laminography — the Fourier
  USFFT `LamFourierRec` — landed early in M6; see the `lamino` bullet above.)
...
6. ✅ **M3 end-to-end pipeline integration test** — done
   (`tests/pipeline_e2e.rs`; HDF in → preprocess → center 63.500 → FBP r=0.8702

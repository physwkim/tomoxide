...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ⬜ Remaining: (`RayProject` is host-side sparse rows for ART/BART — not a GPU
    ...
    recons ported.**
- **M7 — Beam hardening, AI center (`find_center_sift`), f16, zarr,
  benchmarks.** (Laminography — the Fourier USFFT `LamFourierRec` — landed early
  in M6; see the `lamino` bullet above.)

...
- **M6 — wgpu/Metal:** runs the GPU path on Apple Silicon (Metal), Vulkan/DX12
  ...
  - ⬜ Remaining: **GPU FBP filter apply** (now unblocked: pad → fft_1d →
    ×filter → ifft → crop, all on GPU) to close the full GPU FBP recon path;
    then Bluestein for non-power-of-two FFT. (`RayProject` is host-side sparse
    rows for ART/BART — not a GPU kernel.)

...
| `recon::lprec`               | tomocupy `reconstruction/lprec.py:292`; `cuda/cfunc_lprec.cu`, `include/cfunc_lprec.cuh:9` | CUDA, wgpu | stub — **currently routed through the FBP filter+back-project path** (a known divergence: lprec is genuinely a log-polar method). Its distinct USFFT log-polar port is pending |

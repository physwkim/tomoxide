...
| `recon::linerec`             | tomocupy `reconstruction/linerec.py:47`; `cuda/cfunc_linerec.cu`, `include/cfunc_linerec.cuh:9` | CPU | Routed through the FBP filter+back-project path, which is faithful: `cfunc_linerec`'s `backprojection_ker` with the standard tomography tilt (φ=π/2) reduces to parallel-beam back-projection with linear interpolation (scaled `4/nproj`), i.e. a filtered back-projection. Laminographic φ≠π/2 (the `_lamino` kernel) is a separate `recon::lamino` concern |
| `recon::filter` (FBP filter) | tomocupy `reconstruction/fbp_filter.py:46`; `cuda/cfunc_filter.cu`; tomopy filter in `fbp.c` | all | CPU + wgpu done. wgpu `Fft` primitive: 1-D handles **any length** — power-of-two runs the direct radix-2 kernel (fft.wgsl bit-reversal + per-stage butterflies), other lengths run the Bluestein chirp-z transform (bluestein.wgsl spectral multiply + 3 radix-2 FFTs of `m=next_pow2(2n−1)`; host chirp with `j² mod 2n` reduction, rel error ≈1e-7 vs rustfft); 2-D also handles **any dims** — power-of-two runs the on-device transpose fast path, other dims a separable pair of 1-D passes (radix-2 or Bluestein per axis) with a host transpose between (per-axis 1/cols·1/rows compose to 1/(rows·cols)), forward rel ≈1e-7 vs rustfft. Tolerance parity vs rustfft. `FbpFilter::apply` on GPU: each detector lane padded to `pad=filter.len()` then forward FFT → ×filter multiply (fbp_filter.wgsl `apply_filter`, complex×real broadcast) → inverse FFT, all one serialized submission chain, crop to n_cols ×(1/pad); power-of-two pad only (else error→CPU fallback); tolerance parity (2e-3). `make_filter` is shared host arithmetic (`tomoxide_core::backend::make_fbp_filter`) so CPU/GPU build the identical kernel. **Rotation-center handling — intentional mechanism deviation (verified equivalent).** tomocupy's `fbp_filter_center` (backproj_functions.py:103) folds the center into this filter as a per-row Fourier phase `exp(-2πi·(-center+sht+n/2)·t)` (band-limited sub-pixel shift to `n/2`), so its back-projector assumes center `n/2`. tomoxide keeps `apply` a pure shift-invariant ramp and handles the center **downstream**, where each method already touches the data: fbp/linerec sample the back-projector at `t=…+center` (linear-interp sub-pixel); fourierrec/lprec/gridrec apply a signed-frequency Fourier recenter in their own grids. Numerically equivalent and verified against the tomopy goldens (gridrec r=0.98; the `gridrec_subpixel_center` non-integer-center regression test confirms sub-pixel centers don't collapse). Not a defect — a single-site-vs-downstream placement difference; the `FbpFilter::apply` trait contract documents it. The one genuine numerical gap left in `apply` is edge vs zero padding (tomocupy edge-replicates the first/last column before the ramp; tomoxide zero-pads) — a small ramp-ringing difference, deferred |

#!/usr/bin/env python
"""Generate the golden for tomoxide's generalized-Paganin (`GPaganin`) phase test.

The reference is tomocupy `processing/retrieve_phase.py::paganin_filter(method=
'Gpaganin')` (Paganin et al. 2020). tomocupy runs on the GPU via cupy, which is
unavailable on this machine, so this script is a FAITHFUL LINE-BY-LINE
TRANSCRIPTION of tomocupy's exact functions with `cp` -> `np`: `_calc_pad`,
`_calc_pad_val`, `_calc_pad_width`, `_reciprocal_coord`, `_reciprocal_gridG`,
`_paganin_filter_factorG`, and `_retrieve_phase`. `cupy.fft` and `numpy.fft`
...
Run under any env with numpy (e.g. the tomopy-golden env):
    /Users/stevek/mamba/envs/tomopy-golden/bin/python \
        tools/gen_tomocupy_gpaganin_golden.py

Writes, under crates/tomoxide/tests/fixtures/:
  - gpaganin_input.npy      (nproj, dy, dz)  input radiograph stack, float32
  - tomocupy_gpaganin.npy   (nproj, dy, dz)  GPaganin output, float32

Params are tomocupy defaults: pixel_size=1e-4, dist=50, energy=20, db=1000, W=2e-4.
"""
import os
...
def gpaganin_filter(data, pixel_size, dist, energy, db, W, pad=True):
    """tomocupy `paganin_filter(method='Gpaganin')` (retrieve_phase.py:59-108)."""
    py, pz, val = _calc_pad(data, pixel_size, dist, energy, pad)
    dx, dy, dz = data.shape
    kf = _reciprocal_gridG(pixel_size, dy + 2 * py, dz + 2 * pz)
    phase_filter = fftshift(_paganin_filter_factorG(energy, dist, kf, pixel_size, db, W))
...
# Same deterministic radiograph stack as the Paganin test (moving Gaussian blob
# plus a left/right-asymmetric ramp so the edge-pad value is exercised).
...
phase = gpaganin_filter(
    stack.copy(), pixel_size=1e-4, dist=50, energy=20, db=1000, W=2e-4, pad=True
).astype("float32")

np.save(os.path.join(out, "gpaganin_input.npy"), stack)
np.save(os.path.join(out, "tomocupy_gpaganin.npy"), phase)
